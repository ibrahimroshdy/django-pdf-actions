...
class ExportPDFSettings(TimeStampedModel):
    title = models.CharField(max_length=100, help_text=_("Name of this configuration"))
    active = models.BooleanField(
        default=False, help_text=_("Only one configuration can be active at a time")
    )
    ...
    grid_line_color = ColorField(
        default="#000000", help_text=_("Grid line color (hex format, e.g. #000000)")
    )
    ...
    show_header = models.BooleanField(
        default=True, help_text=_("Display the header with model name")
    )
    show_logo = models.BooleanField(
        default=True, help_text=_("Display the logo in the PDF")
    )
    show_export_time = models.BooleanField(
        default=True, help_text=_("Display export timestamp")
    )
    show_page_numbers = models.BooleanField(
        default=True, help_text=_("Display page numbers")
    )
    ...
    def clean(self):
        if self.active:
            ...
            if active_configs.exists():
                raise ValidationError(
                    _(
                        "There can only be one active configuration. Please deactivate "
                        "the current active configuration first."
                    )
                )

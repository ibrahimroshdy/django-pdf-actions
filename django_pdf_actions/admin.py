...
class PdfAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "header_alignment",
        "modified",
        "header_background_color_preview",
        "grid_line_color_preview",
    )
    list_filter = (
        "active",
        "show_header",
    ...
    @admin.display(description="Font")
    def font_name_display(self, obj):
        font_name = os.path.splitext(obj.font_name)[0]  # Remove .ttf extension
        return format_html('<span style="font-family: monospace;">{}</span>', font_name)
    ...
    @admin.display(description="Header Background")
    ...
    @admin.display(description="Grid Line Color")
    ...
    fieldsets = (
        ("General", {"fields": ("title", "active")}),
        ("Page Layout", {"fields": ("page_size", "items_per_page", "page_margin_mm")}),
        (
            "Font Settings",
        ...
        (
            "Alignment Settings",
            {"fields": ("title_alignment", "header_alignment", "content_alignment")},
        ),
        ("Table Settings", {"fields": ("table_spacing", "max_chars_per_line")}),

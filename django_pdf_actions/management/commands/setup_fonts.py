...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--font-url", type=str, help="URL to download additional font from"
        )
        parser.add_argument(
            "--font-name",
            type=str,
    ...
    def download_and_process_font(self, url, target_path, font_name):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".tmp") as temp_file:
            ...
            try:
                # Open as zip if not detected (some servers omit content-type)
                if is_zip or zipfile.is_zipfile(temp_file.name):
                    with zipfile.ZipFile(temp_file.name) as zip_ref:
                        ...
                        ttf_files = [
                            f for f in zip_ref.namelist() if f.lower().endswith(".ttf")
                        ]
                        if not ttf_files:
                        ...
                        if font_name.lower() == "dejavusans.ttf":
                            target_ttf = next(
                                f for f in ttf_files if "DejaVuSans.ttf" in f
                            )
                        else:
                        ...
                        with zip_ref.open(target_ttf) as source, open(
                            target_path, "wb"
                        ) as target:
                            shutil.copyfileobj(source, target)
                            self.stdout.write(
                                f"Extracted {target_ttf} from zip to {target_path}"
                            )
                else:
    ...
    def handle(self, *args, **options):
        ...
        if options["font_url"]:
            ...
            fonts.append({"name": font_name, "url": options["font_url"]})
            self.stdout.write(
                self.style.NOTICE(
                    f"Adding custom font: {font_name} from {options['font_url']}"
                )
            )
        ...
        for font in fonts:
            ...
            if os.path.exists(font_path):
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Font {font['name']} already exists at {font_path}"
                    )
                )
                continue
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully installed {font['name']} to {font_path}"
                    )
                )
                ...
                self.stdout.write(
                    self.style.NOTICE(
                        "You can manually download the font from "
                        f"{font['url']} and place it in {fonts_dir}"
                    )
                )

        self.stdout.write(
            self.style.SUCCESS(f"Font setup complete. Fonts directory: {fonts_dir}")
        )
